...
|------|-------------|-----|
| `daphne_reserve` | **~1.5 GB** | Headroom for the web server. Presigned uploads (FIMEVAL-BE16) keep file bytes out of Daphne, so its footprint is now small and stable; 1.5 GB is comfortable. Verify on your host (below). |
| `os_reserve` | **~1 GB** | Kernel, shell, MinIO client, misc. |
| `per_worker_memory` | **≥ 6 GB** | Heavy methods peak ~4.5 GB per job. 6 GB keeps a worker below Dask's ~80 % pause threshold while leaving room for the nanny to restart a runaway. |
...
headroom (round up). Post-BE16 this is typically a few hundred MB, so 1.5 GB is
